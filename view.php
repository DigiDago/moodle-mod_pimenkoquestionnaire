...
if (!$pimenkoquestionnaire->is_active()) {
    ...
    $pimenkoquestionnaire->page->add_to_page('message', get_string('notopen', 'pimenkoquestionnaire', userdate($pimenkoquestionnaire->opendate)));

} else if ($pimenkoquestionnaire->is_closed()) {
    $pimenkoquestionnaire->page->add_to_page('message', get_string('closed', 'pimenkoquestionnaire', userdate($pimenkoquestionnaire->closedate)));
    ...
    if ($pimenkoquestionnaire->questions) { // Sanity check.
        if (!$pimenkoquestionnaire->user_has_saved_response($USER->id)) {
            $pimenkoquestionnaire->page->add_to_page('complete',
                ...
                'id='.$pimenkoquestionnaire->cm->id).'">'.get_string('answerquestions', 'pimenkoquestionnaire').'</a>');
        } else {
            $resumesurvey = get_string('resumesurvey', 'pimenkoquestionnaire');
            $pimenkoquestionnaire->page->add_to_page('complete',
                '<a href="'.$CFG->wwwroot.htmlspecialchars('/mod/pimenkoquestionnaire/complete.php?' .
                'id='.$pimenkoquestionnaire->cm->id.'&resume=1').'" title="'.$resumesurvey.'">'.$resumesurvey.'</a>');
        }
    } else {
...
if ($pimenkoquestionnaire->capabilities->editquestions && !$pimenkoquestionnaire->questions && $pimenkoquestionnaire->is_active()) {
    $pimenkoquestionnaire->page->add_to_page('complete',
        ...
        'id='.$pimenkoquestionnaire->cm->id).'">'.'<strong>'.get_string('addquestions', 'pimenkoquestionnaire').'</strong></a>');
}
...
$event = \mod_pimenkoquestionnaire\event\course_module_viewed::create(array(
...
if ($pimenkoquestionnaire->capabilities->readownresponses && ($usernumresp > 0)) {
    ...
    $pimenkoquestionnaire->page->add_to_page('yourresponse',
        '<a href="'.$CFG->wwwroot.htmlspecialchars('/mod/pimenkoquestionnaire/myreport.php?'.$argstr).'">'.$titletext.'</a>');
}
