...
$url = new moodle_url('/mod/pimenkoquestionnaire/show_nonrespondents.php', array('id' => $cm->id));
$PAGE->set_url($url);
...
if ($action == 'sendmessage' && !empty($subject) && !empty($message)) {
    ...
    if (is_array($messageuser)) {
        ...
        $url = new moodle_url('/mod/pimenkoquestionnaire/view.php', array('id' => $cm->id));
        redirect($url, $msg, 4);
        exit;
    }
}
...
if (!$nonrespondents) {
    ...
    $pimenkoquestionnaire->page->add_to_page('formarea', html_writer::start_tag('form',
        ['class' => 'mform', 'action' => 'show_nonrespondents.php', 'method' => 'post', 'id' => 'pimenkoquestionnaire_sendmessageform']));
    ...
    if ($fullname) {
        ...
        foreach ($nonrespondents as $nonrespondent) {
            ...
            $data = array ($pimenkoquestionnaire->renderer->user_picture($user, array('courseid' => $course->id)), $profilelink);
        ...
        if (isset($table)) {
            $pimenkoquestionnaire->page->add_to_page('formarea', $pimenkoquestionnaire->renderer->flexible_table($table, $buffering));
        } else if ($buffering) {
            ob_end_clean();
        }
        ...
        if ($showall) {
            ...
            $pimenkoquestionnaire->page->add_to_page('formarea', $pimenkoquestionnaire->renderer->container(html_writer::link($allurl,
        ...
        if (has_capability('mod/pimenkoquestionnaire:message', $context)) {
            ...
            $pimenkoquestionnaire->page->add_to_page('formarea', '<legend>'.get_string('send_message_to', 'pimenkoquestionnaire').'</legend>');
    ...
    if (has_capability('mod/pimenkoquestionnaire:message', $context)) {
        ...
        if ($action == 'sendmessage' && (empty($subject) || empty($message))) {
            $pimenkoquestionnaire->page->add_to_page('formarea', $pimenkoquestionnaire->renderer->notification(get_string('allfieldsrequired')));
        }
        $pimenkoquestionnaire->page->add_to_page('formarea',
        ...
        $table->data[] = array( '<strong>'.get_string('subject', 'pimenkoquestionnaire').'</strong>', $subjecteditor);
        ...
        $module = array('name' => 'mod_pimenkoquestionnaire', 'fullpath' => '/mod/pimenkoquestionnaire/module.js');
