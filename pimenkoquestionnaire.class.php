...
class pimenkoquestionnaire {

    // Class Properties.

    /**
     * @var \mod_pimenkoquestionnaire\question\base[] $quesitons
     */
    public $questions = [];
    ...
    public function __construct($id = 0, $pimenkoquestionnaire = null, &$course, &$cm, $addquestions = true) {
        ...
        if ($id) {
            $pimenkoquestionnaire = $DB->get_record('pimenkoquestionnaire', ['id' => $id]);
        }

        if (is_object($pimenkoquestionnaire)) {
            $properties = get_object_vars($pimenkoquestionnaire);
        ...
        $this->course = $course;
        $this->cm = $cm;
        // When we are creating a brand new pimenkoquestionnaire, we will not yet have a context.
        ...
        // Load the capabilities for this user and pimenkoquestionnaire, if not creating a new one.
        if (!empty($this->cm->id)) {
            $this->capabilities = pimenkoquestionnaire_load_capabilities($this->cm->id);
        }
    }
    ...
    public function add_survey($sid = 0, $survey = null) {
        ...
        if ($sid) {
            $this->survey = $DB->get_record('pimenkoquestionnaire_survey', ['id' => $sid]);
    ...
    public function add_questions($sid = false) {
        ...
        $params = [$sid, 'n'];
        if ($records = $DB->get_records_select('pimenko_question', $select, $params, 'position')) {
            $sec = 1;
            $isbreak = false;
            foreach ($records as $record) {

                $this->questions[$record->id] = \mod_pimenkoquestionnaire\question\base::question_builder($record->type_id,
                        $record, $this->context);
    ...
    /**
     * Add the renderer to the pimenkoquestionnaire object.
     *
    ...
    /**
     * Add the templatable page to the pimenkoquestionnaire object.
     *
    ...
    */
    ...
    public function view() {
        ...
        $PAGE->requires->js_init_call('M.mod_pimenkoquestionnaire.init_attempt_form', null, false,
                pimenkoquestionnaire_get_js_module());

        $pimenkoquestionnaire = $this;

        if (!$this->capabilities->view) {
            $this->page->add_to_page('notifications',
                    $this->renderer->notification(get_string('noteligible', 'pimenkoquestionnaire', $this->name),
                            \core\output\notification::NOTIFY_ERROR));
        } else if (!$this->is_active()) {
            $this->page->add_to_page('notifications',
                    $this->renderer->notification(get_string('notavail', 'pimenkoquestionnaire'),
                            \core\output\notification::NOTIFY_ERROR));
        } else if (!$this->is_open()) {
            $this->page->add_to_page('notifications',
                    $this->renderer->notification(get_string('notopen', 'pimenkoquestionnaire', userdate($this->opendate)),
                            \core\output\notification::NOTIFY_ERROR));
        } else if ($this->is_closed()) {
            $this->page->add_to_page('notifications',
                    $this->renderer->notification(get_string('closed', 'pimenkoquestionnaire', userdate($this->closedate)),
                            \core\output\notification::NOTIFY_ERROR));
        } else if (!$this->user_is_eligible($USER->id)) {
            $this->page->add_to_page('notifications',
                    $this->renderer->notification(get_string('noteligible', 'pimenkoquestionnaire'),
                            \core\output\notification::NOTIFY_ERROR));
        } else if ($this->survey->realm == 'template') {
            $this->page->add_to_page('notifications',
                    $this->renderer->notification(get_string('templatenotviewable', 'pimenkoquestionnaire'),
                            \core\output\notification::NOTIFY_ERROR));
            ...
            $this->page->add_to_page('notifications',
                    $this->renderer->notification(get_string('alreadyfilled', 'pimenkoquestionnaire', $msgstring),
                            \core\output\notification::NOTIFY_ERROR));
        } else {
            // Handle the main pimenkoquestionnaire completion page.
            $quser = $USER->id;
            ...
            $viewform = data_submitted($CFG->wwwroot . "/mod/pimenkoquestionnaire/complete.php");
            ...
            if (data_submitted() && confirm_sesskey() && isset($viewform->submit) && isset($viewform->submittype) &&
                ...
                if ($this->grade != 0) {
                    ...
                    $pimenkoquestionnaire->id = $this->id;
                    $pimenkoquestionnaire->name = $this->name;
                    $pimenkoquestionnaire->grade = $this->grade;
                    $pimenkoquestionnaire->cmidnumber = $this->cm->idnumber;
                    $pimenkoquestionnaire->courseid = $this->course->id;
                    pimenkoquestionnaire_update_grades($pimenkoquestionnaire, $quser);
                }
                ...
                $params = [
                        ...
                        'anonymous' => $anonymous,
                        'other' => ['pimenkoquestionnaireid' => $pimenkoquestionnaire->id]
                ];
                $event = \mod_pimenkoquestionnaire\event\attempt_submitted::create($params);
                $event->trigger();
    ...
    */
    ...
    public function user_time_for_new_attempt($userid) {
        ...
        if (!($attempts = $DB->get_records('pimenko_response', $params, 'submitted DESC'))) {
            return true;
        }
    ...
    public function print_survey($userid = false, $quser) {
        ...
        // If student saved a "resume" pimenkoquestionnaire OR left a pimenkoquestionnaire unfinished
        ...
        $msg = '';
        $action = $CFG->wwwroot . '/mod/pimenkoquestionnaire/complete.php?id=' . $this->cm->id;
        ...
        if ($formdata->sec == 1) {
            $SESSION->pimenkoquestionnaire->end = false;
        }

        if (!empty($formdata->submit)) {
            // Skip logic: we have reached the last page without any questions on it.
            if (isset($SESSION->pimenkoquestionnaire->end) && $SESSION->pimenkoquestionnaire->end == true) {
                return;
            }
        ...
        // Save each section 's $formdata somewhere in case user returns to that page when navigating the pimenkoquestionnaire.
            ...
            if ($msg) {
                ...
                if ($this->has_dependencies()) {
                    while (!$this->eligible_questions_on_page($formdata->sec, $formdata->rid)) {
                        $this->response_delete($formdata->rid, $formdata->sec);
                        $formdata->sec++;
                        // We have reached the end of pimenkoquestionnaire on a page without any question left.
                        if ($formdata->sec > $numsections) {
                            $SESSION->pimenkoquestionnaire->end =
                                    true; // End of pimenkoquestionnaire reached on a no questions page.
                            break;
                        }
                    }
                }
        ...
        if (!empty($formdata->prev)) {
            ...
            // unlock pimenkoquestionnaire->end to allow navigate back to previous page.
            if (isset($SESSION->pimenkoquestionnaire->end) && ($SESSION->pimenkoquestionnaire->end == true)) {
                $SESSION->pimenkoquestionnaire->end = false;
                $formdata->sec--;
            }
        ...
        if (isset($this->questions) && $numsections) { // Sanity check.
            ...
            if ($formdata->sec > 1) {
                $controlbuttons['prev'] =
                        ['type' => 'submit', 'value' => '<< ' . get_string('previouspage', 'pimenkoquestionnaire')];
            }
            if ($this->resume) {
                $controlbuttons['resume'] = ['type' => 'submit', 'value' => get_string('save', 'pimenkoquestionnaire')];
            }
            ...
            if ($formdata->sec == $numsections) {
                ...
                $controlbuttons['submit'] = ['type' => 'submit', 'value' => get_string('submitsurvey', 'pimenkoquestionnaire')];
            } else {
                $controlbuttons['next'] = ['type' => 'submit', 'value' => get_string('nextpage', 'pimenkoquestionnaire') . ' >>'];
            }
            ...
            $this->page->add_to_page('controlbuttons',
                    $this->renderer->complete_controlbuttons(get_string('noneinuse', 'pimenkoquestionnaire')));
        }
    ...
    private function get_response($userid, $rid = 0) {
        ...
        if ($rid != 0) {
            // Check for valid rid.
            $fields = 'id, userid';
            $params = ['id' => $rid, 'pimenkoquestionnaireid' => $this->id, 'userid' => $userid, 'complete' => 'n'];
            return ($DB->get_record('pimenko_response', $params, $fields) !== false) ? $rid : '';

        } else {
            // Find latest in progress rid.
            $params = ['pimenkoquestionnaireid' => $this->id, 'userid' => $userid, 'complete' => 'n'];
            if ($records = $DB->get_records('pimenko_response', $params, 'submitted DESC', 'id,pimenkoquestionnaireid', 0, 1)) {
                $rec = reset($records);
                return $rec->id;
            } else {
                return '';
    ...
    private function response_select_max_sec($rid) {
        ...
        $max = $DB->count_records_select('pimenko_question', $select, $params) + 1;
    ...
    private function response_select_max_pos($rid) {
        ...
        foreach (['response_bool', 'resp_single', 'resp_multiple', 'response_rank', 'response_text',
            ...
            $sql = 'SELECT MAX(q.position) as num FROM {pimenko_' . $tbl . '} a, {pimenko_question} q ' .
    ...
    private function response_check_format($section, $formdata, $checkmissing = true, $checkwrongformat = true) {
        ...
        foreach ($this->questionsbysec[$section] as $question) {
            ...
            if (!$question->response_complete($formdata)) {
                $missing++;
                $strmissing .= get_string('num', 'pimenkoquestionnaire') . $qnum . '. ';
            }
            if (!$question->response_valid($formdata)) {
                $wrongformat++;
                $strwrongformat .= get_string('num', 'pimenkoquestionnaire') . $qnum . '. ';
            }
        }
        $message = '';
        $nonumbering = false;
        ...
        if ($checkmissing && $missing) {
            ...
            if ($missing == 1) {
                $message = get_string('missingquestion', 'pimenkoquestionnaire') . $strmissing;
            } else {
                $message = get_string('missingquestions', 'pimenkoquestionnaire') . $strmissing;
            }
            if ($wrongformat) {
                $message .= '<br />';
            }
        ...
        if ($checkwrongformat && $wrongformat) {
            if ($nonumbering) {
                $message .= get_string('wronganswers', 'pimenkoquestionnaire');
            } else {
                if ($wrongformat == 1) {
                    $message .= get_string('wrongformat', 'pimenkoquestionnaire') . $strwrongformat;
                } else {
                    $message .= get_string('wrongformats', 'pimenkoquestionnaire') . $strwrongformat;
                }
            }
        }
        return ($message);
    ...
    private function response_delete($rid, $sec = null) {
        ...
        foreach (['response_bool', 'resp_single', 'resp_multiple', 'response_rank', 'response_text',
                'response_other', 'response_date'] as $tbl) {
            $DB->delete_records_select('pimenko_' . $tbl, $select, $params);
        }
    }
    ...
    public function response_insert($section, $rid, $userid, $resume = false) {
        ...
        if (empty($rid)) {
            ...
            $record->pimenkoquestionnaireid = $this->id;
            $record->userid = $userid;
            $rid = $DB->insert_record('pimenko_response', $record);
        } else {
            $record->id = $rid;
            $DB->update_record('pimenko_response', $record);
        }
        if ($resume) {
            ...
            $params = [
                    ...
                    'anonymous' => $anonymous,
                    'other' => ['pimenkoquestionnaireid' => $this->id]
            ];
            $event = \mod_pimenkoquestionnaire\event\attempt_saved::create($params);
            $event->trigger();
        }
    ...
    /**
     * Check if current pimenkoquestionnaire has dependencies set and any question has dependencies.
     *
    ...
    private function response_select($rid, $col = null, $csvexport = false, $choicecodes = 0, $choicetext = 1) {
        ...
        $values = \mod_pimenkoquestionnaire\response\boolean::response_select($rid, $col, $csvexport, $choicecodes, $choicetext);

        // Response_single (radio button or dropdown).
        $values += \mod_pimenkoquestionnaire\response\single::response_select($rid, $col, $csvexport, $choicecodes, $choicetext);

        // Response_multiple.
        $values += \mod_pimenkoquestionnaire\response\multiple::response_select($rid, $col, $csvexport, $choicecodes, $choicetext);

        // Response_rank.
        $values += \mod_pimenkoquestionnaire\response\rank::response_select($rid, $col, $csvexport, $choicecodes, $choicetext);

        // Response_text.
        $values += \mod_pimenkoquestionnaire\response\text::response_select($rid, $col, $csvexport, $choicecodes, $choicetext);

        // Response_date.
        $values += \mod_pimenkoquestionnaire\response\date::response_select($rid, $col, $csvexport, $choicecodes, $choicetext);
    ...
    private function survey_render($section = 1, $message = '', &$formdata) {
        ...
        if ($section > $numsections) {
            ...
            $this->page->add_to_page('notifications',
                    $this->renderer->notification(get_string('finished', 'pimenkoquestionnaire'),
        ...
        foreach ($this->questionsbysec[$section] as $question) {
            ...
            // Need pimenkoquestionnaire id to get the pimenkoquestionnaire object in sectiontext (Label) question class.
            $formdata->pimenkoquestionnaire_id = $this->id;
    ...
    private function print_survey_start(
            $message, $section, $numsections, $hasrequired, $rid = '', $blankpimenkoquestionnaire = false
    ) {
        global $CFG, $DB;
        ...
        if ($rid) {
            $courseid = $this->course->id;
            if ($resp = $DB->get_record('pimenko_response', ['id' => $rid])) {
                if ($this->respondenttype == 'fullname') {
                    $userid = $resp->userid;
                    // Display name of group(s) that student belongs to... if pimenkoquestionnaire is set to Groups separate or visible.
                    ...
                    $event = \mod_pimenkoquestionnaire\event\response_viewed::create($params);
                    $event->trigger();
                }
            }
        }
        $ruser = '';
        if ($resp && !$blankpimenkoquestionnaire) {
            if ($userid) {
            ...
            if ($this->respondenttype == 'anonymous') {
                ...
                if ($resp->submitted) {
                    $timesubmitted =
                            '&nbsp;' . get_string('submitted', 'pimenkoquestionnaire') . '&nbsp;' . userdate($resp->submitted);
                }
            }
        }
        if ($ruser) {
            $respinfo = get_string('respondent', 'pimenkoquestionnaire') . ': <strong>' . $ruser . '</strong>';
            if ($this->survey_is_public()) {
                // For a public pimenkoquestionnaire, look for the course that used it.
                $coursename = '';
                $sql = 'SELECT q.id, q.course, c.fullname ' .
                        'FROM {pimenko_response} qr ' .
                        'INNER JOIN {pimenkoquestionnaire} q ON qr.pimenkoquestionnaireid = q.id ' .
        ...
        if ($this->capabilities->printblank && $blankpimenkoquestionnaire && $section == 1) {
            ...
            $url = '/mod/pimenkoquestionnaire/print.php?qid=' . $this->id . '&amp;rid=0&amp;' . 'courseid=' . $this->course->id .
                    '&amp;sec=1';
    ...
    private function print_survey_end($section, $numsections) {
        ...
        if ($numsections > 1) {
            ...
            $this->page->add_to_page('pageinfo',
                    $this->renderer->container(get_string('pageof', 'pimenkoquestionnaire', $a) . '&nbsp;&nbsp;', 'surveyPage'));
        }
    }
    ...
    private function response_commit($rid) {
        ...
        return $DB->update_record('pimenko_response', $record);
    }
    ...
    private function submission_notify($rid) {
        ...
        if (isset($this->survey)) {
            if (isset($this->survey->email)) {
                ...
                $email = $DB->get_field('pimenkoquestionnaire_survey', 'email', ['id' => $this->survey->id]);
            }
        } else {
            $email = '';
        }
    ...
    private function response_send_email($rid, $email) {
        ...
        $subject = get_string('surveyresponse', 'pimenkoquestionnaire') . ": $name [$rid]";
        $url = $CFG->wwwroot . '/mod/pimenkoquestionnaire/report.php?action=vresp&amp;sid=' . $this->survey->id .
                '&amp;rid=' . $rid . '&amp;instance=' . $this->id;

        // Html and plaintext body.
        $bodyhtml = '<a href="' . $url . '">' . $url . '</a>' . $endhtml;
        $bodyplaintext = $url . $endplaintext;
        $bodyhtml .= get_string('surveyresponse', 'pimenkoquestionnaire') . ' "' . $name . '"' . $endhtml;
        $bodyplaintext .= get_string('surveyresponse', 'pimenkoquestionnaire') . ' "' . $name . '"' . $endplaintext;
    ...
    public function generate_csv($rid = '', $userid = '', $choicecodes = 1, $choicetext = 0, $currentgroupid, $showincompletes = 0
        ...
        $config = get_config('pimenkoquestionnaire', 'downloadoptions');
        ...
        $idtocsvmap = [
                ...
                '1',    // 9: date -> string
                '0',     // 10: numeric -> number.
                '0'     // 11: teacherselect -> string.
        ];

        if (!$survey = $DB->get_record('pimenkoquestionnaire_survey', ['id' => $this->survey->id])) {
    ...
    // Blankpimenkoquestionnaire : if we are printing a blank pimenkoquestionnaire.
    ...
    protected function get_survey_all_responses($rid = '', $userid = '', $groupid = false, $showincompletes = 0) {
        ...
        // If a pimenkoquestionnaire is "public", and this is the master course, need to get responses from all instances.
        if ($this->survey_is_public_master()) {
            $qids = array_keys($DB->get_records('pimenkoquestionnaire', ['sid' => $this->sid], 'id'));
        } else {
            $qids = $this->id;
        }

        foreach ($uniquetypes as $type) {
            $question = \mod_pimenkoquestionnaire\question\base::question_builder($type);
            if (!isset($question->response)) {
                continue;
            }
    ...
    protected function choice_types() {
        return [QUESRADIO, QUESDROP, QUESCHECK, QUESRATE, QUESTEACHERSELECT];
    }
    ...
    protected function process_csv_row(
        ...
        // Moodle:
        // Get the course name that this pimenkoquestionnaire belongs to.
        if (!$this->survey_is_public()) {
            $courseid = $this->course->id;
        ...
        if ($isanonymous) {
            ...
            $fullname = get_string('anonymous', 'pimenkoquestionnaire') . $anonumap[$user->id];
            $username = '';
            $uid = '';
        } else {
            $uid = $user->id;
        ...
        if (in_array('submitted', $options)) {
            ...
            $submitted = date(get_string('strfdateformatcsv', 'pimenkoquestionnaire'), $resprow->submitted);
    ...
    /**
     * Return all the fields to be used for users in pimenkoquestionnaire sql.
     *
     * @return array|string
     * @author: Guy Thomas
     */
    ...
    private function send_submission_notifications($rid) {
        ...
        if ($notifyusers = $this->get_notifiable_users($USER->id)) {
            ...
            $info->submissionurl = $CFG->wwwroot . '/mod/pimenkoquestionnaire/report.php?action=vresp&sid=' . $this->survey->id .
                    '&rid=' . $rid . '&instance=' . $this->id;
            $info->coursename = $this->course->fullname;

            $info->postsubject = get_string('submissionnotificationsubject', 'pimenkoquestionnaire');
            $info->posttext = get_string($langstringtext, 'pimenkoquestionnaire', $info);
            $info->posthtml = '<p>' . get_string($langstringhtml, 'pimenkoquestionnaire', $info) . '</p>';
            if (!empty($message)) {
    ...
    private function get_full_submission_for_notifications($answers) {
        ...
        foreach ($responses as $response) {
            ...
            $message .= get_string('answers', 'pimenkoquestionnaire') . ":<br />\n";
    ...
    private function response_goto_thankyou() {
        ...
        if ($result = $DB->get_record_select('pimenkoquestionnaire_survey', $select, null, $fields)) {
        ...
        if (empty($thankhead)) {
            $thankhead = get_string('thank_head', 'pimenkoquestionnaire');
        }
        ...
        $this->page->add_to_page('addinfo',
                format_text(file_rewrite_pluginfile_urls($thankbody, 'pluginfile.php',
                        $this->context->id, 'mod_pimenkoquestionnaire', 'thankbody', $this->survey->id), FORMAT_HTML,
                        ['noclean' => true]));
        ...
        if ($this->capabilities->readownresponses) {
            $this->page->add_to_page('message',
                    ('<a href="' . $CFG->wwwroot . '/mod/pimenkoquestionnaire/myreport.php?id=' .
    ...
    public function view_all_responses($resps) {
        ...
        if ($resps) {
            ...
            $this->page->add_to_page('responses',
                    $this->renderer->all_response_output(get_string('noresponses', 'pimenkoquestionnaire')));
        }
    ...
    public function can_view_response($rid) {
        ...
        if (!empty($rid)) {
            $response = $DB->get_record('pimenko_response', ['id' => $rid]);
            ...
            if ($response->pimenkoquestionnaireid != $this->id) {
                return false;
            }
            ...
            if ($this->capabilities->readallresponses &&
                    ($this->resp_view == QUESTIONNAIRE_STUDENTVIEWRESPONSES_ALWAYS ||
                            ...
                            ($this->resp_view == QUESTIONNAIRE_STUDENTVIEWRESPONSES_WHENANSWERED &&
                                    !$this->user_can_take($USER->id)))) {
                return true;
            }
    ...
    public function count_submissions($userid = false, $groupid = 0) {
        ...
        // Since submission can be across pimenkoquestionnaires in the case of public pimenkoquestionnaires, need to check the realm.
        // Public pimenkoquestionnaires can have responses to multiple pimenkoquestionnaire instances.
        if ($this->survey_is_public_master()) {
            $sql = 'SELECT COUNT(r.id) ' .
                    'FROM {pimenko_response} r ' .
                    'INNER JOIN {pimenkoquestionnaire} q ON r.pimenkoquestionnaireid = q.id ' .
                    'INNER JOIN {pimenkoquestionnaire_survey} s ON q.sid = s.id ' .
                    $groupsql .
            ...
            $sql = 'SELECT COUNT(r.id) ' .
                    'FROM {pimenko_response} r ' .
                    $groupsql .
                    'WHERE r.pimenkoquestionnaireid = :pimenkoquestionnaireid AND r.complete = :status' . $groupcnd;
            $params['pimenkoquestionnaireid'] = $this->id;
            $params['status'] = 'y';
        }
        if ($userid) {
    ...
    /**
     * True if the user can view the responses to this pimenkoquestionnaire, and there are valid responses.
     *
    ...
    /**
     * True if the user can view all of the responses to this pimenkoquestionnaire any time, and there are valid responses.
     *
     * @param null $usernumresp
     *
     * @return bool
    ...
    public function survey_print_render($message = '', $referer = '', $courseid, $rid = 0, $blankpimenkoquestionnaire = false) {
        ...
        if (!empty($rid)) {
            ...
            $this->view_response($rid, $referer, $blankpimenkoquestionnaire);
            return;
        }
        ...
        $action = $CFG->wwwroot . '/mod/pimenkoquestionnaire/preview.php?id=' . $this->cm->id;
        ...
        if (data_submitted()) {
            ...
            foreach ($this->questionsbysec as $section) {
                ...
                if ($errormessage) {
                    if ($numsections > 1) {
                        $pageerror = get_string('page', 'pimenkoquestionnaire') . ' ' . $s . ' : ';
                    }
                    $this->page->add_to_page('notifications',
        ...
        if ($errors == 0) {
            $this->page->add_to_page('message',
                    $this->renderer->notification(get_string('submitpreviewcorrect', 'pimenkoquestionnaire'),
                            \core\output\notification::NOTIFY_SUCCESS));
        }
        ...
        foreach ($this->questionsbysec as $section) {
            $output = '';
            if ($numsections > 1) {
                $output .= $this->renderer->print_preview_pagenumber(get_string('page', 'pimenkoquestionnaire') . ' ' . $page);
                $page++;
            }
        ...
        // End of questions.
        if ($referer == 'preview' && !$blankpimenkoquestionnaire) {
            $url = $CFG->wwwroot . '/mod/pimenkoquestionnaire/preview.php?id=' . $this->cm->id;
            $this->page->add_to_page('formend',
                    $this->renderer->print_preview_formend($url, get_string('submitpreview', 'pimenkoquestionnaire'),
                            get_string('reset')));
        }
        return;
    }
    ...
    public function view_response(
            $rid, $referer = '', $blankpimenkoquestionnaire = false, $resps = '', $compare = false,
        ...
        if ($referer != 'print') {
            ...
            if ($this->survey->feedbacknotes) {
                $text = file_rewrite_pluginfile_urls($this->survey->feedbacknotes, 'pluginfile.php',
                        $this->context->id, 'mod_pimenkoquestionnaire', 'feedbacknotes', $this->survey->id);
    ...
    public function response_analysis(
        ...
        require_once($CFG->dirroot . '/mod/pimenkoquestionnaire/drawchart.php');

        // Find if there are any feedbacks in this pimenkoquestionnaire.
        $sql = "SELECT * FROM {pimenko_fb_sections} WHERE surveyid = ? AND section IS NOT NULL";
        ...
        if ($this->survey->feedbackscores) {
            ...
            $table->wrap = [];
            if ($compare) {
                $table->head = [get_string('feedbacksection', 'pimenkoquestionnaire'), $ruser, $groupname];
            } else {
                $table->head = [get_string('feedbacksection', 'pimenkoquestionnaire'), $groupname];
            }
        }
        ...
        // Calculate max score per question in pimenkoquestionnaire.
        $qmax = [];
        $maxtotalscore = 0;
        foreach ($this->questions as $question) {
            $qid = $question->id;
        ...
        if ($this->survey->feedbacksections == 1) {
            ...
            $feedbacks = $DB->get_records('pimenko_feedbackections', ['sectionid' => $sectionid]);
            $labels = [];
            ...
            $feedback = $DB->get_record_select('pimenko_feedbackections',
            ...
            $sectionheading = file_rewrite_pluginfile_urls($sectionheading, 'pluginfile.php',
                    $this->context->id, 'mod_pimenkoquestionnaire', 'sectionheading', $sectionid);
            ...
            if (!empty($feedback->feedbacktext)) {
                ...
                $feedbacktext = file_rewrite_pluginfile_urls($feedback->feedbacktext, 'pluginfile.php',
                        $this->context->id, 'mod_pimenkoquestionnaire', 'feedback', $feedback->id);
        ...
        for ($section = 1; $section <= $numsections; $section++) {
            ...
            if (!$allresponses) {
                ...
                $formatoptions->noclean = true;
                $sectionheading = file_rewrite_pluginfile_urls($sectionheading, 'pluginfile.php',
                        $this->context->id, 'mod_pimenkoquestionnaire', 'sectionheading', $imageid);
    ...
    public function survey_update($sdata) {
        ...
        if (empty($this->survey->id)) {
            ...
            $this->survey->id = $DB->insert_record('pimenkoquestionnaire_survey', $record);
            $this->add_survey($this->survey->id);

            if (!$this->survey->id) {
                $errstr = get_string('errnewname', 'pimenkoquestionnaire') . ' [ :  ]'; // TODO: notused!
                return (false);
            }
        } else {
            ...
            $name = $DB->get_field('pimenkoquestionnaire_survey', 'name', ['id' => $this->survey->id]);

            // Trying to change survey name.
            if (trim($name) != trim(stripslashes($sdata->name))) {  // $sdata will already have slashes added to it.
                $count = $DB->count_records('pimenkoquestionnaire_survey', ['name' => $sdata->name]);
                if ($count != 0) {
                    $errstr = get_string('errnewname', 'pimenkoquestionnaire');  // TODO: notused!
                    return (false);
                }
            ...
            $result = $DB->update_record('pimenkoquestionnaire_survey', $surveyrecord);
            if (!$result) {
                $errstr = get_string('warning', 'pimenkoquestionnaire') . ' [ :  ]';  // TODO: notused!
                return (false);
            }
        }
    ...
    public function survey_copy($owner) {
        ...
        $i = 0;
        $name = $survey->name;
        while ($DB->count_records('pimenkoquestionnaire_survey', ['name' => $name]) > 0) {
            $name = $survey->name . (++$i);
        }
        if ($i) {
            $survey->name .= $i;
        ...
        // Create new survey.
        if (!($newsid = $DB->insert_record('pimenkoquestionnaire_survey', $survey))) {
            return (false);
        }
        ...
        $dependquestions = $DB->get_records('pimenko_dependency', ['surveyid' => $this->survey->id], 'questionid');
        foreach ($dependquestions as $dquestion) {
            ...
            $DB->insert_record('pimenko_dependency', $record);
        }

        // Replicate any feedback data.
        // TODO: Need to handle image attachments (same for other copies above).
        $fbsections = $DB->get_records('pimenko_fb_sections', ['surveyid' => $this->survey->id], 'id');
        foreach ($fbsections as $fbsid => $fbsection) {
            ...
            unset($fbsection->id);
            $newfbsid = $DB->insert_record('pimenko_fb_sections', $fbsection);
            $feedbackrecs = $DB->get_records('pimenko_feedbackections', ['sectionid' => $fbsid], 'id');
            foreach ($feedbackrecs as $feedbackrec) {
                $feedbackrec->sectionid = $newfbsid;
                unset($feedbackrec->id);
                $DB->insert_record('pimenko_feedbackections', $feedbackrec);
            }
        }
    ...
    public function survey_results_navbar_alpha($currrid, $currentgroupid, $cm, $byresponse) {
        ...
        $url = $CFG->wwwroot . '/mod/pimenkoquestionnaire/report.php?action=vresp&group=' . $currentgroupid .
        ...
        if (!$byresponse) {     // Display navbar.
            ...
            $url = $CFG->wwwroot . '/mod/pimenkoquestionnaire/report.php?action=vresp&byresponse=1&group=' . $currentgroupid;
            // Display navbar.
            $navbar->listlink = $url;

            // Display a "print this response" icon here in prevision of total removal of tabs in version 2.6.
            $linkname = '&nbsp;' . get_string('print', 'pimenkoquestionnaire');
            $url = '/mod/pimenkoquestionnaire/print.php?qid=' . $this->id . '&rid=' . $currrid .
                    '&courseid=' . $this->course->id . '&sec=1';
            $title = get_string('printtooltip', 'pimenkoquestionnaire');
            ...
            for ($i = 0; $i < $total; $i++) {
                if ($isfullname) {
                    ...
                    $resparr[] = '<a title = "' . $responsedate . '" href="' . $url . '&amp;rid=' .
                            ...
                            get_string('response', 'pimenkoquestionnaire') . ($i + 1) . '</a> ';
                }
            }
    ...
    */
    ...
    public function survey_results(
            $precision = 1, $showtotals = 1, $qid = '', $cids = '', $rid = '',
            $uid = false, $currentgroupid = '', $sort = ''
    ) {
        global $SESSION, $DB;

        $SESSION->pimenkoquestionnaire->noresponses = false;
        if (empty($precision)) {
            $precision = 1;
        }
        if ($showtotals === '') {
        ...
        $haschoices = [];
        $responsetable = [];
        if (!($types = $DB->get_records('pimenko_question_type', [], 'typeid', 'typeid, has_choices, response_table'))) {
            $errmsg = sprintf('%s [ %s: question_type ]',
                    get_string('errortable', 'pimenkoquestionnaire'), 'Table');
            return ($errmsg);
        }
        ...
        if (empty($this->survey)) {
            $errmsg = get_string('erroropening', 'pimenkoquestionnaire') . " [ ID:{$this->sid} R:";
            return ($errmsg);
        }

        if (empty($this->questions)) {
            $errmsg = get_string('erroropening', 'pimenkoquestionnaire') . ' ' . 'No questions found.';
            return ($errmsg);
        }
        ...
        if (!empty($rid)) {
            ...
            if (!$rows) {
                $this->page->add_to_page('respondentinfo',
                        $this->renderer->notification(get_string('noresponses', 'pimenkoquestionnaire'),
                                \core\output\notification::NOTIFY_ERROR));
                $SESSION->pimenkoquestionnaire->noresponses = true;
                return;
            }
            $numresps = count($rows);
            $this->page->add_to_page('respondentinfo',
                    ' ' . get_string('responses', 'pimenkoquestionnaire') . ': <strong>' . $numresps . '</strong>');
            if (empty($rows)) {
                $errmsg = get_string('erroropening', 'pimenkoquestionnaire') . ' ' .
                        get_string('noresponsedata', 'pimenkoquestionnaire');
                return ($errmsg);
            }
        ...
        foreach ($this->questions as $question) {
            ...
            $this->page->add_to_page('responses',
                    $this->renderer->container(format_text(file_rewrite_pluginfile_urls($question->content, 'pluginfile.php',
                            $question->context->id, 'mod_pimenkoquestionnaire', 'question', $question->id),
        ...
        if (is_array($questions)) {
            ...
            foreach ($questions as $question) {
                ...
                if ($question->id == $movequestion->id) {
                    ...
                    $DB->update_record("pimenko_question", $movequestion);
                    continue;
                }
                $question->position = $index;
                $DB->update_record("pimenko_question", $question);
                $index++;
            }
            return true;
        }
