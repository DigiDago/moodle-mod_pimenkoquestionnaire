...
/**
 * Privacy test for the mod pimenkoquestionnaire.
 *
...
use \mod_pimenkoquestionnaire\privacy\provider;
 ...
 */
class mod_pimenkoquestionnaire_privacy_testcase extends \core_privacy\tests\provider_testcase {
    /**
     * Tests set up.
     */
    ...
    /**
     * Test that only users with a pimenkoquestionnaire context are fetched.
     */
    public function test_get_users_in_context() {
        ...
        $userlist = new \core_privacy\local\request\userlist(context_system::instance(), 'mod_pimenkoquestionnaire');
        provider::get_users_in_context($userlist);
    ...
    public function test_export_user_data() {
        ...
        $approvedlist = new core_privacy\local\request\approved_contextlist($user, 'mod_pimenkoquestionnaire', [$cmcontext->id]);
        provider::export_user_data($approvedlist);
        $data = $writer->get_data([]);

        $this->assertContains($pimenkoquestionnaire->name, strip_tags($data->name));
        $this->assertEquals($pimenkoquestionnaire->intro, strip_tags($data->intro));
    ...
    public function test_delete_data_for_all_users_in_context() {
        ...
        $this->assertCount(2, $DB->get_records('pimenko_response', []));

        // Delete everything for the context.
        provider::delete_data_for_all_users_in_context($cmcontext);
        $this->assertCount(0, $DB->get_records('pimenko_response', []));
    }
    ...
    public function test_delete_data_for_user() {
        ...
        $approvedlist = new \core_privacy\local\request\approved_contextlist($user, 'pimenko_response', [$cmcontext->id]);
        provider::delete_data_for_user($approvedlist);

        $this->assertCount(0, $DB->get_records('pimenko_response', ['userid' => $user->id]));

        // Get all accounts. There should be one.
        $this->assertCount(1, $DB->get_records('pimenko_response', []));
    }
