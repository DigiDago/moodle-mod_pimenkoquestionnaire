...
Feature: In pimenkoquestionnaire, personality tests can be constructed using feedback on specific question responses.
  In order to define a feedback pimenkoquestionnaire
  As a teacher
  I must add the required question types and complete the feedback options.

  @javascript
  Scenario: Create a pimenkoquestionnaire with a rate question type and verify that feedback options exist.
    ...
    Then I should see "Feedback options are available if your pimenkoquestionnaire contains the following question types"
