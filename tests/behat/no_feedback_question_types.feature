...
Feature: In pimenkoquestionnaire, certain questions will not activate feedback options.
  In order to prevent feedback options
  As a teacher
  I must not add question types that activate feedback.

  @javascript
  Scenario: Create a pimenkoquestionnaire with no feedback questions types.
