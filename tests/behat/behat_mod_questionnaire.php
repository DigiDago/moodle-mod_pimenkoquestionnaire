...
class behat_mod_pimenkoquestionnaire extends behat_base {
    ...
    public function has_questions_and_responses($questionnairename) {
        ...
        if (!$questionnaire = $DB->get_record('pimenkoquestionnaire', array('name' => $questionnairename), 'id,sid')) {
            throw new ExpectationException('Invalid pimenkoquestionnaire name specified.', $this->getSession());
        }

        if (!$DB->record_exists('pimenkoquestionnaire_survey', array('id' => $questionnaire->sid))) {
    ...
    private function add_question_data($sid) {
        ...
        $this->add_data($choicedata, 'pimenkoquestionnaire_quest_choice', 'choicemap', array('questionmap' => 'question_id'));
    }
