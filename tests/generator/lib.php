...
/**
 * mod_pimenkoquestionnaire data generator
 *
...
use mod_pimenkoquestionnaire\generator\question_response,
        mod_pimenkoquestionnaire\generator\question_response_rank,
        mod_pimenkoquestionnaire\question\base;

global $CFG;
require_once($CFG->dirroot . '/mod/pimenkoquestionnaire/locallib.php');

class mod_pimenkoquestionnaire_generator extends testing_module_generator {
    ...
    /**
     * Create a pimenkoquestionnaire with questions and response data for use in other tests.
     */
    public function create_test_questionnaire( $course, $qtype = null, $questiondata = [], $choicedata = null ) {
        ...
        $cm = get_coursemodule_from_instance('pimenkoquestionnaire', $questionnaire->id);
        if ($qtype !== null) {
        ...
        $questionnaire = new pimenkoquestionnaire($questionnaire->id, null, $course, $cm, true);
        return $questionnaire;
    }

    /**
     * Create a pimenkoquestionnaire activity.
     *
    ...
    public function create_instance( $record = null, array $options = null ) {
        ...
        $cm = get_coursemodule_from_instance('pimenkoquestionnaire', $instance->id);
        $course = get_course($cm->course);
        $questionnaire = new pimenkoquestionnaire(0, $instance, $course, $cm, false);
     ...
     */
    public function create_question( pimenkoquestionnaire $questionnaire, $record = null, $data = null ) {
        global $DB;
        ...
        if ($typeid !== QUESPAGEBREAK && $typeid !== QUESSECTIONTEXT) {
            $qtype = $DB->get_record('pimenkoquestionnaire_question_type', ['id' => $typeid]);
            if (!$qtype) {
        ...
        // Add the question.
        $record->id = $DB->insert_record('pimenkoquestionnaire_question', $record);

        $question = \mod_pimenkoquestionnaire\question\base::question_builder($record->type_id, $record->id, $record);
     ...
     *
     * @param \mod_pimenkoquestionnaire\question\base $question
     * @param stdClass                                $data
     */
    ...
    /**
     * Create a survey instance with data from an existing pimenkoquestionnaire object.
     *
    ...
    public function create_content( $questionnaire, $record = [] ) {
        ...
        $survey = $DB->get_record('pimenkoquestionnaire_survey', ['id' => $questionnaire->sid], '*', MUST_EXIST);
    ...
    public function create_question_response( $questionnaire, $question, $respval, $userid = 1, $section = 1 ) {
        ...
        return $DB->get_record('pimenkoquestionnaire_response', ['id' => $responseid]);
    }

    /**
     * Need to create a method to access a private pimenkoquestionnaire method.
     * TO DO - may not need this with above "TO DO".
     */
    private function response_commit( $questionnaire, $responseid ) {
        $method = new ReflectionMethod('pimenkoquestionnaire', 'response_commit');
        $method->setAccessible(true);
        return $method->invoke($questionnaire, $responseid);
    }

    public function create_and_fully_populate(
            $coursecount = 4, $studentcount = 20, $questionnairecount = 2,
            $questionspertype = 5
    ) {
        global $DB;
        ...
        $questiontypes = [QUESTEXT, QUESESSAY, QUESNUMERIC, QUESDATE, QUESRADIO, QUESDROP, QUESCHECK, QUESRATE, QUESTEACHERSELECT];
        $students = [];
        $courses = [];
        $questionnaires = [];
    ...
    public function question_has_choices( $typeid ) {
        $choicequestions = [QUESCHOOSE, QUESRADIO, QUESCHECK, QUESDROP, QUESRATE, QUESTEACHERSELECT];
    ...
    public function generate_response( $questionnaire, $questions, $userid, $complete = true ) {
        ...
        return $this->create_response(['pimenkoquestionnaireid' => $questionnaire->id, 'userid' => $userid], $responses, $complete);
    }

    /**
     * Create response to pimenkoquestionnaire.
     *
     * @param array|stdClass $record
    ...
    public function create_response( $record = null, $questionresponses, $complete = true ) {
        ...
        if (!isset($record['pimenkoquestionnaireid'])) {
            throw new coding_exception('pimenkoquestionnaireid must be present in phpunit_util::create_response() $record');
        }
        ...
        // Add the response.
        $record['id'] = $DB->insert_record('pimenkoquestionnaire_response', $record);
        ...
        $DB->update_record('pimenkoquestionnaire_response', $record);
    ...
    protected function add_response_choice( $questionresponse, $responseid ) {
        ...
        $question = $DB->get_record('pimenkoquestionnaire_question', ['id' => $questionresponse->questionid]);
        ...
        if ($qtype === QUESCHOOSE || $qtype === QUESTEACHERSELECT || $qtype === QUESRADIO || $qtype === QUESDROP ||
            ...
            if ($qtype == QUESRATE) {
                $DB->insert_record('pimenkoquestionnaire_response_rank', [
            ...
            $DB->insert_record('pimenkoquestionnaire_response_text', [
                            'response_id' => $responseid,
                            'question_id' => $questionresponse->questionid,
