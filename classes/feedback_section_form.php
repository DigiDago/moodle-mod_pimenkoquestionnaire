...
class feedback_section_form extends \moodleform {
    ...
    public function definition() {
        ...
        if ($feedbacksections == 1) {
            $label = get_string('feedbackglobal', 'pimenkoquestionnaire');
            $feedbackheading = get_string('feedbackglobalheading', 'pimenkoquestionnaire');
            $feedbackmessages = get_string('feedbackglobalmessages', 'pimenkoquestionnaire');
        } else {
            $label = $feedbacksection->sectionlabel;
            $feedbackheading = get_string('feedbacksectionheading', 'pimenkoquestionnaire', $label);
            $feedbackmessages = get_string('feedbackmessages', 'pimenkoquestionnaire', $label);
        }
        ...
        if ($pimenkoquestionnaire->survey->feedbacksections > 0) {
            // Sections.
            if ($survey->feedbacksections > 1) {
                ...
                if (!empty($feedbacksection->scorecalculation)) {
                    ...
                    foreach ($feedbacksection->scorecalculation as $qid => $score) {
                        ...
                        $mform->addGroup($questionactions, '', $pimenkoquestionnaire->questions[$qid]->name);
                        $counter++;
                    }
                }
                if (!empty($qvalid)) {
        ...
        $mform->addHelpButton('feedbackhdr', 'feedback', 'pimenkoquestionnaire');

        $mform->addElement('static', 'scoreboundarystatic1', get_string('feedbackscoreboundary', 'pimenkoquestionnaire'), '100%');
    ...
    public function validation( $data, $files ) {
        ...
        while (!empty($data['feedbackboundaries'][$i])) {
            ...
            if (is_numeric($boundary) && $boundary <= 0) {
                $errors["feedbackboundaries[$i]"] = get_string('feedbackerrorboundaryoutofrange', 'pimenkoquestionnaire', $i + 1);
            }
            if (is_numeric($boundary) && $i > 0 &&
                    $boundary >= $data['feedbackboundaries'][$i - 1]) {
                $errors["feedbackboundaries[$i]"] = get_string('feedbackerrororder', 'pimenkoquestionnaire', $i + 1);
            }
