...
class single extends base {
            ...
            foreach ($this->question->choices as $cid => $choice) {
                if (strpos($choice->content, '!other') === 0) {
                    ...
                    if (preg_match("/[^ \t\n]/", $other)) {
                        ...
                        $resid = $DB->insert_record('pimenko_response_other', $record);
                        $val = $cid;
                        break;
                    }
                }
    ...
    public function results_template() {
        return 'mod_pimenkoquestionnaire/results_choice';
    }
    ...
    static public function response_select( $rid, $col = null, $csvexport = false, $choicecodes = 0, $choicetext = 1 ) {
        ...
        foreach ($records as $qid => $row) {
            ...
            if ($csvexport) {
                static $i = 1;
                $qrecords = $DB->get_records('pimenko_quest_choice', ['question_id' => $qid]);
                foreach ($qrecords as $value) {
                    if ($value->id == $cid) {
                        $contents = pimenkoquestionnaire_choice_values($value->content);
                        if ($contents->modname) {
                            $row->ccontent = $contents->modname;
                        } else {
                            $content = $contents->text;
                            if (preg_match('/^!other/', $content)) {
                                $row->ccontent = get_string('other', 'pimenkoquestionnaire');
    ...
    public function get_bulk_sql(
      ...
      LEFT JOIN {pimenko_response_other} qro ON qro.response_id = qr.id AND qro.choice_id = qrs.choice_id
