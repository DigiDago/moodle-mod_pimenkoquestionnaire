...
class multiple extends single {
    ...
    static public function response_select($rid, $col = null, $csvexport = false, $choicecodes = 0, $choicetext = 1) {
        ...
        if ($csvexport) {
            ...
            if (!empty($records)) {
                ...
                $sql = 'SELECT * FROM {pimenko_quest_choice} WHERE question_id ' . $qsql . ' ORDER BY id';
                ...
                foreach ($records2 as $qid => $row2) {
                    ...
                    if (preg_match('/^!other/', $c2)) {
                        ...
                        $contents = pimenkoquestionnaire_choice_values($c2);
                        if ($contents->modname) {
                            $c2 = $contents->modname;
    ...
    public function get_bulk_sql($pimenkoquestionnaireids, $responseid = false, $userid = false, $groupid = false, $showincompletes = 0) {
      ...
      LEFT JOIN {pimenko_response_other} qro ON qro.response_id = qr.id AND qro.choice_id = qrm.choice_id
