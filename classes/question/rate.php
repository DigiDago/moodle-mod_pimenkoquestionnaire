...
class rate extends base {
    ...
    protected function responseclass() {
        return '\\mod_pimenkoquestionnaire\\response\\rank';
    }
    ...
    public function question_template() {
        return 'mod_pimenkoquestionnaire/question_rate';
    }
    ...
    public function response_template() {
        return 'mod_pimenkoquestionnaire/response_rate';
    }
            ...
            if (isset($choice->content)) {
                ...
                if ($nbchoices > 1 && $this->precise != 2  && !$blankpimenkoquestionnaire) {
                    ...
                    if ($notcomplete && isset($data->$str) && ($data->$str == -999)) {
                        ...
                        $title = get_string('pleasecomplete', 'pimenkoquestionnaire');
                    }
    ...
    protected function response_survey_display($data) {
        ...
        if ($this->precise == 1) {
            $cellobj = new \stdClass();
            $cellobj->bg = $bg;
            $cellobj->str = get_string('notapplicable', 'pimenkoquestionnaire');
            $resptags->headers[] = $cellobj;
        }
