...
class radio extends base {
    ...
    public function question_template() {
        return 'mod_pimenkoquestionnaire/question_radio';
    }
    ...
    public function response_template() {
        return 'mod_pimenkoquestionnaire/response_radio';
    }
    ...
    protected function question_survey_display($data, $dependants=[], $blankpimenkoquestionnaire=false) {
        ...
        foreach ($this->choices as $id => $choice) {
            ...
            if ($other !== 0) { // This is a normal radio button.
                ...
                $othertext = preg_replace(["/^!other=/", "/^!other/"], ['', get_string('other', 'pimenkoquestionnaire')],
                    $choice->content);
                $cid = 'q'.$this->id.'_'.$id;
                $otherempty = false;
        ...
        if (!$this->required()) {
            ...
            if (!$ischecked && !$blankpimenkoquestionnaire) {
                $radio->checked = true;
            }
    ...
    protected function form_length(\MoodleQuickForm $mform, $helptext = '') {
        $lengroup = [];
        $lengroup[] =& $mform->createElement('radio', 'length', '', get_string('vertical', 'pimenkoquestionnaire'), '0');
        $lengroup[] =& $mform->createElement('radio', 'length', '', get_string('horizontal', 'pimenkoquestionnaire'), '1');
        $mform->addGroup($lengroup, 'lengroup', get_string('alignment', 'pimenkoquestionnaire'), ' ', false);
        $mform->addHelpButton('lengroup', 'alignment', 'pimenkoquestionnaire');
