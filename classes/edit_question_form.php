...
class edit_question_form extends \moodleform {
    ...
    public function definition() {
        ...
        global $pimenkoquestionnaire, $question, $SESSION;

        // TODO - Is there a better way to do this without session global?
        // The 'sticky' required response value for further new questions.
        if (isset($SESSION->pimenkoquestionnaire->required) && !isset($question->qid)) {
            $question->required = $SESSION->pimenkoquestionnaire->required;
        }
        if (!isset($question->type_id)) {
            print_error('undefinedquestiontype', 'pimenkoquestionnaire');
        }

        // Each question can provide its own form elements to the provided form, or use the default ones.
        if (!$question->edit_form($this, $pimenkoquestionnaire)) {
    ...
    public function validation($data, $files) {
        ...
        if ($data['type_id'] == QUESRATE) {
            ...
            if ($data['precise'] == 2 ) {
                ...
                if ($nbvalues < 2) {
                    $errors["allchoices"] = get_string('noduplicateschoiceserror', 'pimenkoquestionnaire');
                }
            }
        }
