...
/**
 * Contains class mod_pimenkoquestionnaire\output\renderer
 *
...
class renderer extends \plugin_renderer_base {
    ...
    /**
     * Fill out the pimenkoquestionnaire (complete) page.
     * @param \templateable $page
     * @return string | boolean
     */
    ...
    public function question_output($question, $formdata, $dependants=[], $qnum, $blankquestionnaire) {
        ...
        return $this->render_from_template('mod_pimenkoquestionnaire/question_container', $pagetags);
    }
    ...
    public function all_response_output($data=null) {
        ...
        if (is_string($data)) {
            ...
            foreach ($data as $qnum => $responses) {
                ...
                $output .= $this->render_from_template('mod_pimenkoquestionnaire/response_container', $pagetags);
            }
        }
        return $output;
    }
        ...
        foreach ($children as $child) {
            ...
            foreach ($child as $subchild) {
                ...
                switch ($subchild->dependlogic) {
                    case 0:
                        $logic = get_string('notset', 'pimenkoquestionnaire');
                        break;
                    case 1:
                        $logic = get_string('set', 'pimenkoquestionnaire');
                        break;
                    default:
                        $logic = '';
                }
                ...
                if (!in_array($subchild->qdependquestion, $loopindicator)) {
                    $msg .= '<div class = "qn-container">'.$strnum.' '.$subchild->position.$childname.
                        ...
                        get_string('dependquestion', 'pimenkoquestionnaire').'</strong>'.
        ...
        foreach ($dependencies as $dependency) {
            ...
            if ($dependency->dependandor == "and") {
                $html .= '<div id="qdepend_' . $qid . '_' . $dependency->dependquestionid . '_' .
                    ...
                    get_string('dependquestion', 'pimenkoquestionnaire') . '</strong> : '. get_string('position', 'pimenkoquestionnaire') . ' ' .
